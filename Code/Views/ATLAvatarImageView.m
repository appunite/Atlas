...
- (void)setAvatarItem:(id<ATLAvatarItem>)avatarItem
{
    ...
    if ([avatarItem respondsToSelector:@selector(placeholderImage)]) {
        placeholder = [avatarItem placeholderImage];
    }
    
    if ([[[avatarItem avatarImageURL] absoluteString] rangeOfString:@"http"].location != NSNotFound) {
        [self setImageWithURL:[avatarItem avatarImageURL] placeholderImage:placeholder];
