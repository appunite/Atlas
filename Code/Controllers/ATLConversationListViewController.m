...
{
    if (queryController == self.searchQueryController) {
        return;
    }
...
{
    if (controller == self.searchQueryController) {
        return;
    }
...
- (NSString *)defaultLastMessageTextForConversation:(LYRConversation *)conversation
{
    ...
    return [[self class] defaultLastMessageTextForMessage:lastMessage];
}

+ (NSString *)defaultLastMessageTextForMessage:(LYRMessage *)lastMessage {
